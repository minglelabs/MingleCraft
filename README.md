...
- Resource/supply checks plus BWAPI capability checks. Baseline providers use bounded candidates; the Jev action generator retains every supported legal candidate. Every executable action is a leaf of the command tree, even though the full action table is omitted from the model request.
- Command kind → acting unit or unit group → concrete action. One Jev request contains all tree questions, the optional Noul win forecast, and independent coordinate-digit questions. Code follows the chosen tree path to one executable leaf.
...
The direct Jev adapter implements the [official TypeSafe HTTP contract](https://docs.typesafe.ai/api): `POST /v1/systemone`, structured `state`, `model`, and typed `questions`. OpenRouter Jev receives the same typed request through its endpoint. An eligible step submits every Choice question and, when enabled, one Noul `win_probability` question together. Questions are independent: Choice answers cannot consume the fresh Noul result. Prior forecasts are labeled as prior estimates. Cumulative history remains local. See [Jev primitives](https://docs.typesafe.ai/primitives) and [the policy contract](docs/jev-policy.md).

All providers receive finite questions. The tree uses command kind, actor, and executable leaf levels; priority and strategy categories do not determine its structure. Every branch is answered independently in the same call, and code uses only answers on the chosen path. Model bias and inconsistent answers between branches remain possible. Heuristic priorities are private to the rule baseline and pruner. Direct Jev and OpenRouter Jev return model probabilities/confidence; rule, random, OpenAI and local providers do not invent calibrated confidence.
...
The bridge sets 42 ms/frame, samples no faster than every 6 game frames, and permits only one decision in flight. **2–4 decisions/second is a target**, not a verified result. The default budgets are 96,000 UTF-8 bytes, 48,000 estimated tokens for the complete request, and 24,000 estimated tokens for state plus any one question. These token estimates are not official tokenizer counts. One-shot questions avoid serial inference but increase request size; if the complete action tree cannot fit, the request is rejected and `context_budget_exceeded` is logged without deleting candidates. Existing orders continue during inference. This Python change requires a service restart; no BWAPI DLL rebuild is required.
