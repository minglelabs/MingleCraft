...
- Resource/supply checks plus BWAPI capability checks. Baseline providers use the bounded candidate path; Jev receives every candidate represented by the supported finite TvT contract, which can exceed 50. Its wire payload uses compact entity/action tables and one node-local question at a time; the full tree is kept local while legal choices and command mapping remain lossless.
- Command kind → acting unit or unit group → concrete action. Jev makes sequential calls per eligible step: a Noul win-probability forecast followed by one Choice question per selected tree node. Both are roles of the same Jev model, not separate trained networks or measured win rates.
...
All providers receive the same state and finite questions. The choice tree uses command kind, actor, and executable leaf levels; priority and strategy categories do not determine its structure. This reduces structural presentation bias, but model bias remains possible. Traversal is greedy, so it has no global-optimality guarantee, and sequential node requests increase latency. Heuristic priorities are private to the rule baseline and pruner. Random is uniform **per hierarchy node**, not over all leaf actions. Direct Jev and OpenRouter Jev return model probabilities/confidence; rule, random, OpenAI and local providers do not invent calibrated confidence.
...
The bridge sets 42 ms/frame, samples no faster than every 6 game frames, and permits only one decision sequence in flight. **2–4 decisions/second is a target**, not a verified result: sequential node inferences, network time, state construction, and candidate count affect throughput. The default conservative request budgets are 96,000 bytes and 24,000 estimated tokens; the token figure is a heuristic estimate, not an official tokenizer count. The formal contract allows roughly 32k state-plus-question tokens and 64k whole-request tokens, leaving headroom for provider framing. Existing orders continue during inference. A shared per-step deadline, request-size guard, and 24-frame expiry prevent late or oversized choices from silently acting on stale state. An extremely large state can still exceed the budget even after node splitting and will fall back safely. This Python change requires a service restart; no BWAPI DLL rebuild is required.
