...
For the first remote setup, do not move the JevCraft server to the Mac. Keep these components together on the home Windows machine:

```text
StarCraft 1.16.1 + BWAPI + JevCraft.dll + Python JevCraft server
```
...
4. In the Mac Windows App client, connect to the Windows PC's Tailscale `100.x.y.z` address or MagicDNS name. Tailscale provides the private path; do not port-forward RDP or JevCraft's port `8765` from the home router.
5. On the Windows desktop, start the JevCraft server locally:
...
Remote desktop and game automation have different failure and security boundaries. JevCraft should own observations, finite decisions, BWAPI commands and evaluation traces. A future companion project, `jevcraft-remote`, can provide a narrow Windows operator agent for:

- health checks and current match status;
- starting or stopping an allowlisted JevCraft process;
- retrieving logs and summaries;
- optional screenshot capture.

That agent should never expose arbitrary shell execution or keyboard/mouse injection over the JevCraft HTTP API. Full desktop control belongs to the authenticated remote desktop layer. The v0.1 repository does not include this companion agent yet.
...
- If the health check fails, start `jevcraft serve` on Windows before launching the match.
- If the health check works but the game shows no actions, check `runs\` on Windows and confirm Chaoslauncher loaded `JevCraft.dll`.
